import { Arg, ID, Info, Mutation, Query, Resolver, Ctx, Authorized } from "type-graphql";
import {
  Category,
  CategoryCreateInput,
  CategoryUpdateInput,
...
export class CategoriesResolver {
  ...
  async deleteCategory(
    ...
    // ici je decide que je ne peux supprimer que les catégories que j'ai créées = sécurité applicative
    const category = await Category.findOneBy({
      id,
      createdBy:{ id: context.user.id },
    });
